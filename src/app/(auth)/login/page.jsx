...
function LoginFormFallback() {
  return (
    <div className="max-w-sm w-full rounded shadow-2xl border-2 border-white grid gap-2 p-4 bg-transparent">
      <h1 className="text-center font-semibold text-3xl">Login</h1>
...
function LoginForm({ refParam }) {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      setIsLoading(false);
      if (refParam) {
        router.push(refParam, { replace: true });
      } else {
        router.push("/", { replace: true });
      }
    } catch (error) {
    ...
    <div className="h-screen w-full flex justify-center items-center bg-gradient-to-t to-red-300 from-pink-400">
      <Suspense fallback={<LoginFormFallback />}>
        <SearchParamsWrapper>
          {(searchParams) => (
            <LoginForm
              refParam={searchParams ? searchParams.get("ref") : null}
            />
          )}
        </SearchParamsWrapper>
      </Suspense>
